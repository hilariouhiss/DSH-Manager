...
|---|---|---|---|---|
| `well` / `well-hover` | `ui/app.slint:241` 的 `touch.has-hover ? #0000001F : #00000024` | `#00000024` / `#0000001F` | `#FFFFFF2E` / `#FFFFFF38` | 暗色是白胶囊里的**黑**井；浅色 CTA 是实心强调色，井要翻成**白**井。⚠ **非悬停是 `24`，悬停是 `1F`** —— 调用点是 `has-hover ? well-hover : well`，这个配对才复现原来的渲染。写反会让暗色悬停变亮，是个静默的外观回退 |
...
`Orb` 的默认 tint（`:136`）改为取 **`Tokens.glow-brand`**，不再是字面量 —— 两个调用点传的都是 `glow-*`
（带 α 的氛围光），缺省必须同档。

⚠ **不要**写成 `Tokens.accent`：accent 是**不透明**色，拿它当 tint 会让任何没显式传 tint 的 Orb
变成一块实心色斑。本规格初稿写的正是 `Tokens.accent`，是被 Task 4 的评审按上述理由否掉的 ——
此处保留这段说明，免得下一个人又"顺手"改回去。
