...
### Ruling 89 — 【**2026-09-22 修订：已实现**，见本节末】系统代理不生效。
...
- **⚠ 2026-09-22 修订（人类裁决：实现它）—— 本条从"已知限制"改成"已实现"。**
  走的是上面自己挑出来的那条**没被依赖挡住**的路：
  `HKCU\Software\Microsoft\Windows\CurrentVersion\Internet Settings` 的
  `ProxyEnable` / `ProxyServer`，用**已启用**的 `Win32_System_Registry` feature
  （与 `theme::spawn_watcher` 同一套 API）—— **不新增依赖、不改 `Cargo.toml`**，
  这正是本节 ①/② 两条理由想要的那个出口。实现在 `dsh::system_proxy()`。
  - **设置面板新增一档「网络代理」**：使用系统代理（缺省）/ 直连。
    作用范围**只有本程序自己的出网**（registry 版本目录 + GitHub 更新说明）——
    这是人类在需求澄清里明确选的。`dsh web` 是**另一个进程**，跟的是它自己的
    环境变量，不受这一档影响；面板上的说明文字必须把这点说出来，否则用户会误判。
  - **优先级定为「环境变量 > 系统代理 > 直连」。** 环境变量在前是刻意的：
    它在本条之前就是唯一生效的方式（V-6 靠注入 `HTTPS_PROXY` 才验成），
    所以这个开关在语义上是**纯加法** —— 只可能多给一个候选，不会从任何
    已经在用 `HTTPS_PROXY` 的人手里拿走那条路。
  - **缺省是"走"而不是"直连"**：`state.json` 里没记过（`None`）一律当走。
    装了没配代理的机器上 `ProxyEnable` 是 0，这一档自动退化成直连 ——
    缺省选它的代价是零；反过来则是把本条描述的那个坑留给每一个没改过设置的用户。
  - **实测（本机）**：`system_proxy()` 读出 `http://127.0.0.1:12450/`，
    与注册表 `ProxyServer` 逐字一致（也就是本节正文里那个"返回 200"的代理）。
  - `PROXY_HINT` **保留**，只是改成同时指向设置面板与 `HTTPS_PROXY`：
    用户把这一档关掉、或目标机器压根没配系统代理时，那句话仍是唯一能解释
    "为什么只有本程序上不了网"的东西。
  - **未做：`ProxyOverride` 绕过列表。** 上限是"该直连的走了代理"（安全的
    那一侧，不是"该走代理的直连了"）；升级路径是 ureq 的
    `ProxyBuilder::no_proxy()`。代码里留了 `ponytail:` 注释标注这条上限。
