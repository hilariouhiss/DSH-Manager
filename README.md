...
- **单实例**：重复启动不会再起一个实例 —— 不管窗口是隐藏到托盘、最小化还是正常开着，只会把**已有的**窗口弹回前台（进程、托盘图标、`state.json` 始终只有一份）。
- **自动更新本程序**：启动时查一次 GitHub Release（帮助菜单里也能手动查）。有新版本时顶栏出现徽标并弹一次提示，〔下载并安装〕会把安装包下到临时目录、**校验 SHA256**、**静默安装进原目录**（只有安装进度窗，全程不用再点）并退出本程序 —— **装完自己回来**，从点下去到新版本跑起来只有那一下点击；`%TEMP%` 里那份安装包也会在重启后自动删掉。也可以〔以后再说〕或〔忽略此版本〕。装更新时**不会**停掉正在运行的 `dsh web`。
...
| 路径                                                   | 内容                                                       |
| ------------------------------------------------------ | ---------------------------------------------------------- |
| `%APPDATA%\dsh-manager\state.json`                     | 端口、关闭行为、主题、代理开关、被忽略的更新版本（原子写入，损坏时回退缺省） |
