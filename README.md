...
- **自动更新本程序**：启动时查一次 GitHub Release（帮助菜单里也能手动查）。有新版本时顶栏出现徽标并弹一次提示，〔下载并安装〕会把安装包下到临时目录、**校验 SHA256**、**静默安装进原目录**（只有安装进度窗，全程不用再点）并退出本程序 —— **装完自己回来**，从点下去到新版本跑起来只有那一下点击。也可以〔以后再说〕或〔忽略此版本〕。装更新时**不会**停掉正在运行的 `dsh web`。
